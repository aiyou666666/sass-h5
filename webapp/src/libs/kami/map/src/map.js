...
var Map = Widget.extend({
    ...
    options: {
        token: 'CA4de4f8d8e3c3891eb39a78f8343cd4',//private token
        type: 'quick',
        version: '1.0',
    ...
    _getAPI: function () {
        ...
        this.scriptEl = el;
        this.baiduScriptPattern = new RegExp('ak\=' + this.get('token'), 'i');
        head.appendChild(el);
    },
    ...
    destroy: function () {
        ...
        if (this.asyn) {
            ...
            //删除 baidu API 在 body 后面插入的多余的 scripts
            var delScriptArr = [];
            for (var i = document.scripts.length - 1; i >= 0; i--) {
                var script = document.scripts[i];
                if(script &&
                    script.src &&
                    script.src.match(this.baiduScriptPattern)){

                    delScriptArr.push(script);
                }
            }

            delScriptArr.forEach(function(script){
