 ...
 * @extends Widget
 * @category primary
 * @demo http://ued.qunar.com/mobile/kami/demo/src/html/panel/index.html
 */
...
var Panel = Widget.extend({
    ...
    _translate: function(translateX, translateY) {
        ...
        this.trigger('scroll', translateX, translateY, this._stopAnimate);
         var durationIsZero = getTransitionTime( this._scroller ) === "0s";
         //当transition-duration为0的时候 即使transform属性发生改变 也不会触发transitionEnd事件
         if (durationIsZero){
             this.trigger('scrollEnd');
         }
    },
...
// 获取元素的getTransitionTime
function getTransitionTime(elem) {
    var matrix = window.getComputedStyle(elem[0], null);
    var transitionDuration = matrix['webkitTransitionDuration'] || matrix['transitionDuration'];
