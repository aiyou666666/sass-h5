 ...
 * @extends List
 * @category primary
 * @demo http://ued.qunar.com/mobile/kami/demo/src/html/pagelist/index.html
 */
...
var Pagelist = List.extend({
    ...
    /**
     * 停止滚动并滚动到指定位置
     * @function stopAndScrollTo
     * @memberOf Pagelist
     * @param  {Number} translateY 指定滚动到的位置
     * @return {Boolean} 是否成功
     */
    stopAndScrollTo: function (translateY) {
        translateY =  translateY || parseInt(translateY, 10);
        if (isNaN(translateY)) {
            console.warn('stopAndScrollTo:translateY必须为数字');
            return false;
        }
        if (this.get('infinite')) {
            ...
            var pagelist = this;
            pagelist.scrollTo(translateY, 300, null, function() {
                pagelist._reorderInfinite();
            });
            return true;
            ...
            this.scrollTo(translateY, 300, null);
        }
    ...
    refresh: function(data, isFail) {
        this._loadEl && this._loadEl.hide();
        if(!isFail) {
            this._pageNum = 1;
            this.reload(data);
            this._setLoadmore();
    ...
    simulateRefresh: function () {
        ...
        // this.scrollTo(refreshActiveY, 0);
        this.scrollTo(0, 300,null,function() {
            self._refreshInit();
        }, 300);
