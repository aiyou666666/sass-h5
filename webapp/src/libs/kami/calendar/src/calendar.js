 ...
 * @extends Widget
 * @category primary
 * @demo http://ued.qunar.com/mobile/kami/demo/src/html/calendar/index.html
 */
...
var Calendar = Widget.extend({
    ...
    init: function () {
        ...
        var formateKey=this.get("frmateKey");
        if(formateKey=="year"){
        	    /*只可以选择年份 */
        	   this.datasource=[datasource[0]]
               this.datasource[0].datasource = _getYearDs(this.dateRange);
             }else if(formateKey=="month"){
             	/*可以选择年和月份 */
        	   this.datasource=[datasource[0],datasource[1]];
        	   this.datasource[0].datasource = _getYearDs(this.dateRange);
        	   this.datasource[1].datasource = _getMonthDs(this.dateRange, this.now, this.now.getFullYear());
	        }else if(formateKey=="date"){
	            /*选择日期 */
    ...
    _changeValue: function (key, curItem, prevItem) {
        ...
        if (key === 'year') {
            ...
            if(formateKey=="date" || formateKey=="month"){
            	...
            	ds = _getDateDs(this.dateRange, this.now, year);
            }
            this.innerSelect.setDataSource('date', ds);
             ...
             if(formateKey=="year" || formateKey=="date"){
             	 var year = this.innerSelect.getValue('year').year.value;
             }
            //var year = this.innerSelect.getValue('year').year.value;
            var month = curItem.value;
            if(formateKey=="year" || formateKey=="date"){
            	var ds = _getDateDs(this.dateRange, this.now,"", month);
            }
