 ...
 * @extends Widget
 * @category business
 * @demo http://ued.qunar.com/mobile/kami/demo/src/html/datepicker/index.html
 */
...
var Panel = require('../../panel/index.js');
...
var DatePicker = Widget.extend({
     ...
     * @property {Number} offsetBegin 距离开始入店可选择的区间，如果设置了beginDate或者endDate那么这个选项就没有意义了，默认90
     * @property {String} beginDate 允许用户选择的开始日期，默认为空
     * @property {String} endDate 允许用户选择的结束日期，默认为空
     * @property {Boolean} uesDefaultCheckDate 如果没有传checkIn和checkOut选项，是否使用默认的 checkIn 和 checkOut，默认为 true
     * @property {String} checkIn 默认的入住日期
     * @property {String} checkOut 默认退房日期
     * @property {String} gapClass 选择区间日期样式，默认为gap
     * @property {Boolean} allowSameCheckInAndOut 是否允许同一天的同学入住
     * @memberOf DatePicker
     */
    ...
    options: {
        ...
        // 结束选择的日期
        endDate: '',
        // 是否使用默认的 checkIn 和 checkOut
        uesDefaultCheckDate: true,
        // 默认checkin的日期
        checkIn: '',
        // 默认checkout的日期
        checkOut: '',
    ...
    init: function() {
        ...
        if (this.get('uesDefaultCheckDate')) {
            this._getCheckDate();
        }
        this._getData();
    ...
    _getCheckDate: function() {
        ...
        if (!this.get('checkOut')) {
            var offsetInteger = parseInt(this.get('offsetCheckIn'), 10);
            //是否允许同一天签入签出，如果允许那么offsetCheckIn可以为0,否则为1
            var minOffsetCheckIn = 1;

            if (this.get('allowSameCheckInAndOut')) {
                minOffsetCheckIn = 0;
            }
            //
            if (!isNaN(offsetInteger) && offsetInteger >= minOffsetCheckIn) {
                this.set('offsetCheckIn', offsetInteger);
            }
            else {
        ...
        if (this.get('checkIn') && this.get('checkOut') && QunarDate.compareDate(QunarDate.parse(this.get('checkIn')), QunarDate.parse(this.get('checkOut'))) >= minGap) {
