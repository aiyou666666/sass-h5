 ...
 * @constructor
 * @category primary
 * @demo http://ued.qunar.com/mobile/kami/demo/src/html/tip/index.html
 * @extends Widget
 */

var Overlay = require('../../overlay/index.js');
...
var Tip = Widget.extend({
     ...
     * @property {String} align  tip组件出现的位置 默认为center  可选值有：top center bottom
     * @property {Array} verticalOffset 当align为top，或者bottom时额外的偏移量，默认为偏移tip高度
     * @memberOf Tip
     */
    ...
    init: function () {
        ...
        this.verticalOffset = this.get('verticalOffset') || ['auto', 'auto'];
    ...
    initUi: function () {
        ...
        this.verticalOffset = this.verticalOffset.map(function (item, index) {
            if (item === 'auto') {
                return dialogOffset.height;
            }
            else {
                return parseInt(item, 10);
            }
        });
        this.set('verticalOffset', this.verticalOffset);
        // this.verticalOffset = this.get('verticalOffset') || ['auto', 'auto'];
        var left = (viewportOffset.width - dialogOffset.width) / 2;
        switch (this.get('align')){
            case 'top':
                var top =  0 + this.verticalOffset[0];
                break;
            case 'center':
                var top = viewportOffset.height / 2 - dialogOffset.height / 2;
                break;
            case 'bottom':
                var top = viewportOffset.height - dialogOffset.height - this.verticalOffset[1];
                break;
        }
        this.widgetEl.css({
            'position': 'fixed',
